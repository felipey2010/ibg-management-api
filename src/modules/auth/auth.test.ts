...
describe('Auth routes smoke', () => {
  ...
  test('GET /api/auth/status returns module ready message', async () => {
...
describe('Auth integration flows', () => {
  ...
  test('register -> returns tokens', async () => {
    const res = await request(app).post('/api/auth/register').send({ email, password, display_name });
  ...
  test('refresh tampered -> 401', async () => {
    const res = await request(app).post('/api/auth/refresh').send({ refreshToken: 'tampered.token.value' });
    expect(res.status).toBe(401);
  });

  test('refresh after revoke -> 401', async () => {
    const payload = jwt.verify(refreshToken as string, env.JWT_SECRET as jwt.Secret) as any;
    await prisma.user_sessions.update({ where: { id: payload.sessionId }, data: { revoked_at: new Date() } });
  ...
  test('logout -> invalidates refresh token', async () => {
    ...
    const res = await request(app).post('/api/auth/logout').send({ refreshToken: freshRefresh });
    expect(res.status).toBe(200);

    const res2 = await request(app).post('/api/auth/refresh').send({ refreshToken: freshRefresh });
...
describe('Auth service unit tests (mocked)', () => {
  ...
  test('registerUser throws 409 when user already exists', async () => {
    jest.spyOn(authRepository, 'findUserByEmail').mockResolvedValue({ id: 'u1', email: 'a@b.com' } as any);
    await expect(authService.registerUser({ email: 'a@b.com', password: 'pass', display_name: 'A' } as any)).rejects.toHaveProperty('statusCode', 409);
  });
  ...
  test('registerUser creates user and returns tokens', async () => {
    ...
    jest.spyOn(authRepository, 'createUser').mockResolvedValue({ id: 'u2', email: 'x@y.com', display_name: 'X', password_hash: 'h', status: 'ACTIVE' } as any);
    jest.spyOn(authRepository, 'createSession').mockResolvedValue({} as any);

    const tokens = await authService.registerUser({ email: 'x@y.com', password: 'pass', display_name: 'X' } as any);
  ...
  test('loginUser rejects invalid credentials', async () => {
    jest.spyOn(authRepository, 'findUserByEmail').mockResolvedValue(null as any);
    await expect(authService.loginUser('no@user.com', 'pass')).rejects.toHaveProperty('statusCode', 401);
  });

  test('loginUser rejects wrong password', async () => {
    jest.spyOn(authRepository, 'findUserByEmail').mockResolvedValue({ id: 'u3', email: 'b@c.com', display_name: 'B', password_hash: 'hash', status: 'ACTIVE' } as any);
    jest.spyOn(bcrypt, 'compare').mockResolvedValue(false as any);
    await expect(authService.loginUser('b@c.com', 'wrong')).rejects.toHaveProperty('statusCode', 401);
  });

  test('loginUser rejects inactive account', async () => {
    jest.spyOn(authRepository, 'findUserByEmail').mockResolvedValue({ id: 'u4', email: 'd@e.com', display_name: 'D', password_hash: 'hash', status: 'INACTIVE' } as any);
    await expect(authService.loginUser('d@e.com', 'pass')).rejects.toHaveProperty('statusCode', 403);
  });

  test('refreshTokens rejects when session not found', async () => {
    const fakeSessionId = 'nonexistent';
    const token = jwt.sign({ sessionId: fakeSessionId } as any, env.JWT_SECRET as jwt.Secret, { expiresIn: '1h' });
